...
function getToneClasses(token: StudioToken) {
  if (token.priorityTone === "blocked") {
    return "border-amber-300 bg-amber-50 text-amber-950 shadow-[0_16px_30px_rgba(217,119,6,0.18)]";
  }

  if (token.priorityTone === "active") {
    return "border-sky-300 bg-sky-50 text-sky-950 shadow-[0_16px_30px_rgba(14,165,233,0.16)]";
  }

  if (token.priorityTone === "done") {
    return "border-emerald-300 bg-emerald-50 text-emerald-950 shadow-[0_16px_30px_rgba(16,185,129,0.16)]";
  }

  return "border-slate-200 bg-white text-slate-900 shadow-[0_12px_26px_rgba(15,23,42,0.08)]";
}
...
function StudioDrawer({
  ...
  return (
    <aside className="studio-drawer flex min-h-[420px] flex-col overflow-hidden rounded-[30px] border border-slate-200/80 bg-white/96 shadow-[0_24px_60px_rgba(15,23,42,0.12)] backdrop-blur">
            ...
            <section className="grid gap-3 rounded-[24px] bg-slate-50 p-4 sm:grid-cols-2">
              <div className="rounded-[20px] bg-white p-3 shadow-sm">
                ...
                <p className="mt-2 text-sm font-medium text-slate-900">
                  {token.ownerLabel || token.ownerRole}
                </p>
...
export default function StudioSurface({
  ...
  const filter = STUDIO_FILTERS.find((item) => item.id === deferredFilterId)!;
  ...
  return (
    <div className="studio-shell min-h-screen px-4 py-5 text-slate-950 sm:px-5 lg:px-6">
      <div className="mx-auto flex max-w-[1660px] flex-col gap-5">
        <header className="studio-hero overflow-hidden rounded-[34px] border border-slate-900/10 px-5 py-5 text-slate-950 shadow-[0_26px_70px_rgba(15,23,42,0.16)] sm:px-6">
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1.55fr)_390px]">
          <section className="overflow-hidden rounded-[34px] border border-slate-200/80 bg-white/74 shadow-[0_22px_60px_rgba(15,23,42,0.12)] backdrop-blur">
                ...
                <div className="relative grid gap-3 md:grid-cols-4 xl:grid-cols-7 xl:grid-rows-3">
                  {filteredStations.map((station) => (
                    <div
                      ...
                      className={cn(
                        "studio-station group relative flex min-h-[170px] cursor-pointer flex-col rounded-[28px] border border-slate-200/80 bg-gradient-to-br px-4 py-4 text-left transition duration-300 hover:-translate-y-1 hover:shadow-[0_22px_48px_rgba(15,23,42,0.12)]",
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-3">
                          <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/85 text-2xl shadow-sm">
                            {station.icon}
                          </div>
                          <div>
                        ...
                        {station.tokens.length > 0 ? (
                          station.tokens.map((token) => (
                            <button
                              ...
                              className={cn(
                                "studio-token relative min-w-[124px] max-w-[188px] rounded-[20px] border px-3 py-2 text-left transition duration-300 hover:-translate-y-1",
                            ...
                            >
                              <div className="flex items-start justify-between gap-2">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-[11px] font-semibold uppercase tracking-[0.18em] text-current/65">
                                    {token.tokenKind}
                                  </span>
                                </div>
                                {token.ownerLabel ? (
                              ...
                              <p className="mt-2 line-clamp-1 text-sm font-semibold text-current">
                                {token.title}
                              </p>
                              <p className="mt-1 line-clamp-1 text-[11px] text-current/70">
                                {token.productLabel}
                              </p>
                              <div className="mt-2 flex items-center justify-between gap-2 text-[11px] text-current/60">
                                <span className="line-clamp-1">{token.subtitle}</span>
                                {token.amountLabel ? <span>{token.amountLabel}</span> : null}
                              </div>
                            </button>
                          ))
                        ) : (
                      ...
                      {station.blockedCount > 0 ? (
                        <div className="mt-3 inline-flex items-center gap-1 rounded-full bg-rose-600/10 px-2.5 py-1 text-[11px] font-medium text-rose-700">
                          <span>•</span>
