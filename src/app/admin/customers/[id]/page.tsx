  ...
  const [{ data: conversations }, { data: leads }, { data: quotes }] =
    await Promise.all([
      ...
      supabase
        .from("leads")
        .select(
          "id, product_type, width_mm, height_mm, qty, status, due_date, design_brief, ai_image_prompt, ai_prompt_snapshot, note_from_form, requested_document_type, billing_entity_type, billing_name, tax_id, billing_address, fulfillment_mode, liff_profile_snapshot, liff_context_snapshot, created_at"
        )
        .eq("customer_id", id)
        .order("created_at", { ascending: false })
        .limit(50),
