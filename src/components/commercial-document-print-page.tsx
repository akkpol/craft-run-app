...
import type { CommercialDocumentPrintModel } from "@/lib/commercial-document-print";

type CommercialDocumentPrintPageProps = {
  printModel: CommercialDocumentPrintModel;
  toolbar?: ReactNode;
};

export default function CommercialDocumentPrintPage({
  printModel,
  toolbar,
}: CommercialDocumentPrintPageProps) {
  return (
    <div className="min-h-screen bg-[#eef3f8] px-3 py-4 text-slate-950 sm:px-4 sm:py-6 print:bg-white print:p-0">
      ...
      {toolbar ? <div className="mx-auto flex w-full max-w-[210mm] justify-end pb-4">{toolbar}</div> : null}
...
function DocumentField({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-0">
      <p className="border-b border-slate-300 pb-1 text-xs font-bold text-slate-600">{label}</p>
