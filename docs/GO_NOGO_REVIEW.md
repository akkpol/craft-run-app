...
| P3-G12 | Change a runtime setting in `/admin/settings` and save it. | Save succeeds and `settings.updated` appears in `action_log`. | Production `/admin/settings` save by allowlisted admin `akkapol.kumpapug@gmail.com` created `settings.updated` row `ACT-20260502-0246` at `2026-05-02T07:05:07.814509+00:00` with payload `changed_fields=["business_name"]`, `app_settings_id="default"`, and `used_schema_fallback=true`; the temporary probe value was restored. | ✅ | Delivery Engineering / 2026-05-02 |
